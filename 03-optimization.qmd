...
**Connect the picture to the mathematics.** Let $w$ be the fraction invested in Fund A. The remaining fraction, $1-w$, goes to B:

$$
\max_{0\leq w\leq1}\;0.08w+0.04(1-w)
\quad\text{subject to}\quad\sigma(w)\leq0.10,
$$

where $\sigma(w)$ is the portfolio standard deviation, calculated from both funds' volatilities and their correlation.
...
**Connect the picture to the mathematics.** Let $C_{\max}$ be the time when the last order finishes. We minimize $C_{\max}$ while respecting two rules: each order finishes cutting before it starts finishing, and operations never overlap on the same machine.

For these three orders, all six possible common orderings can be checked. The 8-hour schedule is optimal: Finishing needs 7 hours of work and cannot start before hour 1. Larger factories require more systematic search; 20 jobs already give $20!\approx2.43\times10^{18}$ possible orderings on one machine.
...
## Business Analytics: Getting More Orders From the Same Budget
...
**Connect the pictures to the mathematics.** Let $s$ and $u$ be search and social spending in thousands of dollars. The curves above use

$$
r_{\mathrm{search}}(s)=900(1-e^{-s/8}),
\qquad r_{\mathrm{social}}(u)=1600\frac{u}{70+u}.
$$

We assume the channel contributions add without overlap and all orders have the same value. The goal is

$$
\max_{s,u\geq0}\;
r_{\mathrm{search}}(s)+r_{\mathrm{social}}(u)
\quad\text{subject to}\quad s+u=100.
$$
