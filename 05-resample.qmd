...
Resampling methods treat an observed sample as a *finite population*, and random samples are generated/resampled from it to estimate population characteristics and make inferences about the sampled population. Some commonly used resampling methods include:
...
"}
Suppose we have a parameter $\theta$ and an estimator $\htt$. The mean square error (MSE) of the estimator is defined as
$$\E[(\htt-\theta )^2 ]= \var(\htt) + [\b(\htt)]^2,$$ where $\b$ is the bias of the estimator.
:::
...
The distribution of the finite population represented by the sample can be regarded as a pseudo-population with similar characteristics as the true popu- lation. By repeatedly generating random samples from this pseudo-population (resampling), the sampling distribution of a statistic can be estimated. Prop- erties of an estimator such as bias or standard error can be estimated by resampling.
