...
Jackknife is similar to **leave-one-out cross-validation** (LOOCV). Let $\bx= (x_1,\dots,x_n)$ be an observed random sample, and denote the $i$th jackknife sample by $\bx_{(i)} = (x_1,\dots,x_{i-1},x_{i+1},\dots,x_n)$, that is, a subset of $\bx$, and the jackknife estimate is the quantity calculated on it. 
...
For the parameter of interest $\theta$, if the statistics is $T(\bx)=: \htt$ is computed on the full samples.

TO fix the idea, the $i$th jackknife sample is written as $x_{(i)}$ given above, that is, the sample with the $i$th observation removed. Let $\htt=T_n(x)$ be the estimator of the parameter of interest $\theta$. The $i$th jackknife replicate is defined as $\hat \theta_{(i)}=T(x_{(i)})$, that is, the estimate computed from the $i$th jackknife sample.

### Jackknife Estimate of Bias

If $\htt$ is a smooth statistic, then $\htt_{(i)} = t(F_{n−1}(x_{(i)}))$, and the jackknife estimate of bias is 
$$
...
$$
\frac{\hat{\theta}-\E[\htt]}{s e(\hat{\theta})} \approx N(0,1)=Z.
$$
...
The basic bootstrap CI interval transforms the distribution of the replicates by **subtracting the observed statistic**. The quantiles of the transformed sample $\htt^\ast-\htt$ are used to determine the confidence limits.

The $100(1 − \alpha)$% confidence limits for the basic bootstrap confidence interval are
$$
\boxed{
(2\htt - \htt^
\ast_{1-\alpha/2},\quad 2\htt - \htt^
\ast_{\alpha/2})}, 
$$
whee re $\htt^\ast_q$ is the $q$th quantile of the bootstrap replicates $\htt^{(1)},\dots ,\htt^{(B)}$.

::: {.callout-note title="Derivation of the Basic Bootstrap CI"}
Let $\bx = (x_1,\dots,x_n)$ be the observed sample, and a statistics $\htt = T_n(\bx)$ be the estimator of the parameter of interest $\theta$ of the sampling distribution. Assume that we have $B$ replicates $\htt^\ast$ from bootstrap.

To obtain a **symmetric** $100(1-\alpha)$% CI for $\theta$, say $(L,U)$, we know that we need 
$$
P(U \le \theta)= P(\theta \le L)=\alpha/2.
$$
 Then, we have 
 $$
\alpha/2 = P(L \ge \theta) = P(L-\htt \ge \theta - \htt) =
P(\htt - L \le \htt - \theta), 
 $$
 where the last equality is to multiply the both side by $-1$. Then
$$
1- \alpha/2 = 1- P(\htt - L \le \htt - \theta) = P(\htt - L \ge  \htt - \theta).
$$
Now, we have to solve it, we need $\htt-L$ be the $1-\alpha/2$'s quantile of the distribution of $\htt - \theta$. However, the distribution of $\htt - \theta$ is *unknown*. In bootstrap, we can approximate the quantile using the bootstrap replicates. 

Therefore, we set the quantile, $\htt - L$ to be the $1-\alpha/2$'s quantile of the distribution of denoted as $\htt^\ast_{1-\alpha/2}$, that is, $\htt^\ast_{1-\alpha/2}-\htt$. This is approximately the $1-\alpha/2$ quantile of $\htt -\theta$. Then, we set $\htt-L$ to be $\htt^\ast_{1-\alpha/2}-\htt$. Hence, from here, we obtain $L = 2\htt - \htt^\ast_{1-\alpha/2}$.
 
Similarly, for the upper limit $U$, we have 
 $$
\alpha/2 = P(\theta \ge U) = P(\theta - \htt \ge U - \htt) = P(\htt - \theta \le \htt - U),
 $$
 which leads to $\htt-U = \htt_{\alpha/2}^\ast - \htt$.
 We *move $\htt$ to other side for both $L$ and $U$. As a result, we obtain
$$
(L,~U) = (2\htt - \htt^\ast_{1-\alpha/2},~2\htt - \htt^\ast_{\alpha/2}),
$$
which is the result of a basic bootstrap CI we state previously.
:::
...
A bootstrap percentile interval uses the empirical distribution of the bootstrap replicates as the reference distribution. The quantiles of the empirical
distribution are estimators of the quantiles of the sampling distribution of $\htt$, so that these (random) quantiles may match the true distribution better when
the distribution of $\htt$ is **not normal**. Suppose that $\{\htt^{(b)}\}_{b=1}^B$ are the bootstrap replicates of the statistic $\htt$. From the ecdf of the replicates, compute the $\alpha/2$ quantile $\htt_{\alpha/2}$, and the $1 − \alpha/2$ quantile $\htt_{1-\alpha/2}$.

Researchers showed that this method some
theoretical advantages over the standard normal interval and somewhat better
coverage performance in practice.

::: {.callout-note title="R command"}
...
3. percentile 
4. studentized
5. BCa (bias-corrected and accelerated), which is an improved percentile intervals that aim to have better theoretical properties and better performance in practice.
:::

::: {.callout-example title="Bootstrap CIs on the patch ratio statistics"}
...
```
  ...
  Recall that the old and new patches are bioequivalent if |θ| ≤ 0.20. Hence, the
interval estimates do not support bioequivalence of the old and new patches.
Next we compute the bootstrap confidence intervals according to their defini-
tions. Compare the following results with the boot.ci output.
...
# display as kable
kable(CI_table, digits = 4, caption = "Bootstrap Confidence Intervals")
```

:::

::: {.callout-note title = "Remark"}
The normal interval computed by boot.ci corrects for bias. Notice
that the boot.ci normal interval differs from our result by the bias
estimate shown in the output from boot. This is confirmed by read-
ing the source code for the function. To view the source code for
this calculation, when the boot package is loaded, enter the command
getAnywhere(norm.ci) at the console. Also see norm.inter and [68]
for details of calculations of quantiles.
:::

::: {.callout-example title="Bootstrap CIs for correlation coefficients"}


Example 8.10 (Bootstrap confidence intervals for the correlation statis-
tic). Compute 95% bootstrap confidence interval estimates for the correlation
statistic in the law data of Example 8.2.
``` {r}
library(boot)
...
All three intervals cover the correlation $\rho=0.76$ of the universe of all law schools in law82. One reason for the difference in the percentile and normal
confidence intervals could be that the sampling distribution of correlation statistic is not close to normal (see the histogram in Figure 8.1). When the sampling distribution of the statistic is approximately normal, the percentile interval will agree with the normal interval. 
:::

### Bootstrap t CI

Even if the distribution of $\htt$ is normal and $\E[\htt]=\theta$, the normal
distribution is not exactly correct for the Z statistic, because we estimate $se(\htt)$. Nor can we claim that it is a Student t statistic, because the distribution
of the bootstrap estimator $se(\htt)$ is unknown. **The bootstrap t interval does not use a Student t distribution as the reference distribution**. Instead, the sampling distribution of a *t type* statistic (a studentized statistic) is generated by resampling. Let $\bx = (x_1, \dots, x_n)$ be an observed sample. The $100(1 − \alpha)$% bootstrap t confidence interval is
$$
\boxed{
\left(\hat{\theta}-t_{1-\alpha / 2}^* \widehat{s e}(\hat{\theta}), \quad \hat{\theta}-t_{\alpha / 2}^* \widehat{s e}(\hat{\theta})\right)},
$$

where $se(\htt),~ t^\ast_{\alpha/2}$ and $t^\ast_{1-\alpha/2}$ are computed as outlined below.

::: {.callout-algorithm title = "Algorithm of Bootstrap t CI"}

1. Compute the observed statistic $\htt$.
2. For each replicate, indexed $b = 1, \dots,B$:
  (a) Sample with replacement from x to get the bth sample $\bx^{(b)} = (x_1^{(b)}, \dots , x_n^{(b)})$ 
  (b) Compute $\htt^{(b)}$ from the $b$th sample $x^{(b)}$.
  (c) Compute or estimate the standard error $\widehat{se}(\htt^{(b)})$ (a separate estimate for each bootstrap sample; a bootstrap estimate will resample
from the current bootstrap sample $\bx^{(b)}$, not $\bx$).
  (d) Compute the $b$th replicate of the *t* statistic,
  $$t(b) = \frac{\htt^{(b)}−\htt}{\widehat{se}(\htt^{(b)})}.$$ 
3. The sample of replicates $t^{(1)},\dots t^{(B)}$ is the reference distribution for bootstrap $t$. Find the sample quantiles $t^\ast_{\alpha/2}$ and $t^\ast_{1-\alpha/2}$ from the ordered sample of replicates $t^{(b)}$.
4. Compute $\widehat{\htt}$, the sample standard deviation of the replicates $\htt^{(b)}$.
5. Compute confidence limits
$$
\left(\hat{\theta}-t_{1-\alpha / 2}^* \widehat{\operatorname{se }}(\hat{\theta}), \quad \hat{\theta}-t_{\alpha / 2}^* \widehat{\operatorname{se}}(\hat{\theta})\right) .
$$
...
To the bootstrap $t$ interval, typically $\widehat{se}(\htt^{(b)})$ must be obtained by bootstrap. This is *a bootstrap nested inside a bootstrap*. If $B = 1000$, for example, the bootstrap t confidence interval method takes approximately $1000$ times longer than any of the other methods.

---

Here, we see how to compute the bootstrap $t$ confidence interval for a univariate or a
multivariate sample $\bx$. The required arguments to the function are 

* the sample data $\bx$,  and
* the function statistic that computes the statistic $T_n(\bx)$. 

The default values are:

* significance level is $\alpha=0.05$
* number of bootstrap replicates $B=500$,
* number of replicates for estimating standard error is $100$.
...
::: {.callout-example title = "Bootstrap t CI for patch ratio statistics"}
...
ci <- boot.t.ci(dat, statistic = stat, B = 2000, 
                R = 200)
print(ci)
```

The upper confidence limit of the bootstrap $t$ confidence interval is much
larger than the three intervals in Example 9 and the bootstrap $t$ is the widest interval here. 
...
- Davison, A.C. and Hinkley, D.V. (1997). [Bootstrap Methods and their Application](https://www.cambridge.org/core/books/bootstrap-methods-and-their-application/ED2FD043579F27952363566DC09CBD6A). Cambridge University Press.
