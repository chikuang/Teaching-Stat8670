...
Suppose we have a parameter $\theta$ and an estimator $\hat \theta$. The mean square error (MSE) of the estimator is defined as $$\mathrm{MSE}_{\theta}(\hat \theta)=\mathbb E[(\hat \theta-\theta )^2 ]= \mathbb{V}ar(\hat \theta) + [\mathbb b(\hat \theta)]^2,$$ where $\b$ is the bias of the estimator.
...
+ 2\,\E\big[(\hat \theta- \E[\hat \theta])(\E[\hat \theta] - \theta)\big]
+ (\E[\hat \theta] - \theta)^2.
$$

The middle term vanishes because $$
\E\big[\hat \theta- \E[\hat \theta]\big] = 0.
$$
...
To estimate the se of an estimator $\htt$, we can use the bootstrap method. Let $\htt^{(1)},\dots,\htt^{(B)}$ be the bootstrap replicates. The bootstrap estimate of the standard error of $\hat \theta$ is given by the sample standard deviation of the replicates:
...
The observed value $\htt$ of the correlation statistic is labeled t1\*. The bootstrap estimate of standard error of the estimate is se(ˆθ) . = 0.13, based on 2000 replicates. To compare with formula (8.1), extract the replicates in \$t, so that the standard deviation of the replicates can be computed directly
...
The bootstrap estimation of bias uses the bootstrap replicates of $\htt$ to estimate the sampling distribution of $\htt$. For the finite population $x = (x_1,\dots , x_n)$, the parameter is $\htt(x)$ and there are $B$ i.i.d. estimators $\htt^{(b)}$. The sample mean of the replicates {ˆθ(b)} is unbiased for its expected value $\E[\htt^\ast]$, so the bootstrap estimate of bias is\
$$bias(\htt) = \htt^\ast − \htt$$, where $\htt^\ast  = \sum_{b=1}^B \hat \theta^{(b)}$, and $\htt= \htt(x)$ is the estimate computed from the original observed sample. (In bootstrap $F_n$ is sampled in place of $F_X$ , so we replace $\theta$ with $\htt$ to estimate the bias.)

::: {.callout-note title = "Remark for estimating bias"}

-   $\oplus$ Positive bias indicates that $\htt$ on average tends to overestimate $\theta$,
...
The `patch` (bootstrap) data contains measurements of a certain hormone in the bloodstream of 8 subjects after wearing
a medical patch. The parameter of interest is

$$
\theta\coloneqq\frac{\E(\text { new })-\E(\text { old })}{\E(\text { old })-\E(\text { placebo })}
$$
