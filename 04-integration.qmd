...
$$
  \E X = \int_{x\in\X} x dF_X(x),\quad \E_{x\in\X} g(X) = \int g(x) dF_X(x).
$$
If density exists, we have
$$
\E X = \int_{x\in\X} x f_X(x) dx,\quad \E g(X) = \int_{x\in\X} g(x) f_X(x) dx.
$$
...
When you want to calculate the inner product of the two observations $x,y\in L^2(\X)$, you need to calculate the integral
...
$$R(\theta) := \E{(X,Y) \sim P}[L(f_\theta(X), Y)],
$$
...
$$\int q_\phi(\theta)\,\log p(x|\theta)\, d\theta,$$
