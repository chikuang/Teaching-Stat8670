...
ITM of generating rvs applies the probability integral transformation. Define the inverse transformation $$ F^{−1}_X(u) = \inf\{x : F_X(x) = u\},\quad  0 < u < 1.$$ Then, if $U \sim \operatorname{Unif}(0,1)$, the rv $X = F^{−1}_X(U)$ has the distribution $F_X$. This can be shown as, for all $x \in \R$
...
2.  Simulate $N=1000$ rvs from $u\sim\unif(0,1)$ and apply the inverse function to obtain the 1000 $x$ values.
...
Q: On average, how many random numbers must be simulated to generate $N=1000$ samples from the $\bet(\alpha=2,\beta=2)$ distribution by ARM?

A: Depends on the upper bound $k$ of $f_X(t)/g_Y(t)$, which depends on the choice of the function $g_Y(\cdot)$.

Recall that the $\bet(2,2)$ density is $$
